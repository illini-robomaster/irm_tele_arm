...
import numpy as np
from thirdparty.dynamixel_sdk.group_sync_read import GroupSyncRead
from thirdparty.dynamixel_sdk.group_sync_write import GroupSyncWrite
from thirdparty.dynamixel_sdk.packet_handler import PacketHandler
from thirdparty.dynamixel_sdk.port_handler import PortHandler
from thirdparty.dynamixel_sdk.robotis_def import (
    COMM_SUCCESS,
    DXL_HIBYTE,
    DXL_HIWORD,
    DXL_LOBYTE,
        ...
        with self._lock:
            for dxl_id in self._ids:
                ...
                if dxl_comm_result != COMM_SUCCESS or dxl_error != 0:
                    warning = 'Failed to set torque mode for Dynamixel with ID ' \
                              f'{dxl_id}: {dxl_comm_result=} {dxl_error=}'
    ...
    def _read_joint_angles(self):
        ...
        while not self._stop_thread.is_set():
            ...
            with self._lock:
                ...
                if dxl_comm_result != COMM_SUCCESS:
                    logger.warning(f'comm failed: {dxl_comm_result}')
                    continue
